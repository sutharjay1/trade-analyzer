...
const portfolioPath = resolve(__dirname, 'portfolio.json');
const portfolioData: Stock[] = JSON.parse(readFileSync(portfolioPath, 'utf-8'));
...
app.get('/stock-data/:symbol', async (req: Request, res: Response) => {
	const stockSymbol: string = req.params.symbol;
	console.log(`Received request for additional stock data: ${stockSymbol}`);
